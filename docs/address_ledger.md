...
|---|---|---|---|---|---|
| `0x821ED038` | Runtime crash site / manual function seed | `sub_821ED038_manual` | Noisy/verbose runtime log failed with ReXGlue fatal: `Call to invalid or unregistered function at guest address 0x821ED038`. Ghidra MCP was attempted first but timed out repeatedly on 2026-05-26. Fallback evidence from generated ReXGlue output shows registered tiny functions at `0x821ED020` and `0x821ED050`; `0x821ED020` is a six-instruction 0x18-byte thunk ending in `bctr`, making `0x821ED038` the same-sized unregistered executable gap before `0x821ED050`. Regenerated output now registers `0x821ED038` and emits a six-instruction vtable thunk that stores `r4` at `r3+324`; the follow-up 60s noisy smoke had `fatal/error/critical_count=0`. | Confirmed | Added `[functions."821ED038"] size = 0x18` in `TheOutFit_Port/config/manual_functions.toml`; this fixed the invalid/unregistered function fatal while preserving first-frame/GPU resolve progress. |
